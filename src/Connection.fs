...
let do_handshake_with_timeout transport protocol =
  with_set_read_timeout
    (Transport.stream transport)
    (fun () -> do_handshake transport protocol)
...
let heartbeat_periodically (T t) =
  ...
  Sequencer.with_
    t
    (fun t -> cleanup t (Transport.Close_reason.errorf "Heartbeat thread stopped: %A" e))
...
let run_after_handshake (T t) reader =
  let result =
    Transport.Reader.read_forever
      reader
      (on_message (T t))
      (fun () -> update_last_seen_alive (T t))
  ...
  let create_and_handshake () =
    Result.let_syntax {
      ...
      | Error error ->
        Sequencer.with_
          t
          (fun t -> cleanup t (Transport.Close_reason.errorf "Handshake failed"))

        return! Or_error.Error.format "Handshake error: %A" error
    }

  Thread.spawn_and_ignore
    "connect then heartbeat loop"
    (fun () ->
